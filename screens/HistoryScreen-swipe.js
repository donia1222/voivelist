...
const HistoryScreen = ({ navigation }) => {
  ...
  const toggleFavorite = (index) => {
    ...
    // Realizar el swipe cuando se presione el botón de favoritos
...
const confirmRemoveListFromHistory = (index) => {
  ...
  const renderHistoryItem = ({ item, index }) => (
    <View style={[
      styles.card, 
      { width: width - 40 }, 
      history.length === 1 && styles.singleCard // Estilo adicional para una sola tarjeta
    ]}>
      ...
      {!favoritesModalVisible ? (
        <TouchableOpacity onPress={() => toggleFavorite(index)} style={styles.headerIconifaco}>
          <Ionicons name={favorites.includes(index) ? "heart" : "heart-outline"} size={28} color="#3f51b5" />
        </TouchableOpacity>
      ) : (
...
<Swiper
  ...
  key={swiperKey}
  cards={history}
  renderCard={(card, index) => card && renderHistoryItem({ item: card, index })}
  onSwiped={(cardIndex) => {
    if (cardIndex === history.length - 1) {
      // Cuando se llega a la última tarjeta, reinicia el Swiper
  ...
  onSwipedAll={() => {
    // Opcional: manejar alguna acción cuando todas las tarjetas hayan sido deslizadas
  }}
  cardIndex={cardIndex}
  ...
  containerStyle={history.length === 1 ? { flex: 1, justifyContent: 'center', alignItems: 'center' } : {}}
/>
